...
import { Utensils, LayoutDashboard, Calendar, ShoppingCart, Settings as SettingsIcon, HelpCircle, LogOut } from 'lucide-react';
...
export default function Sidebar({ activeTab, setActiveTab, onSignOut }: SidebarProps) {
  const navItems = [
    ...
    { id: 'skauppa', icon: ShoppingCart, label: 'S-kauppa' },
  ...
  return (
    <aside className="h-screen w-64 fixed left-0 top-0 border-stone-200/60 flex flex-col p-4 gap-y-2 z-50 border-r border-white/10 bg-sidebar">
      <div className="flex items-center gap-3 px-2 mb-8 mt-2">
        ...
        <div>
          <h1 className="text-xl font-bold tracking-tighter text-white">Culinara</h1>
          <p className="text-[10px] uppercase tracking-widest text-stone-400 font-bold">Event Food Logic</p>
        </div>
      </div>
